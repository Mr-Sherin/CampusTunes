...
export default function DashboardPage() {
  ...
  const handleUploadSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const audioPath = `${Date.now()}_${Math.random().toString(36).substring(7)}.${audioExt}`;
      ...
      // 3. Register Record in Supabase DB
      ...
      const { data: songRecord, error: songInsertErr } = await supabase
        ...
        .insert({
          ...
          artist: fullArtistDisplay,
          album: `${department} • ${releaseType}`,
          genre: genre,
          audio_url: audioUrl,
          cover_url: coverUrl,
          duration: 180,
  ...
  return (
    <div className="space-y-10 pb-16 select-none max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-5 border-b border-white/10">
        <div>
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-xs font-bold text-cyan-300 bg-cyan-500/10 border border-cyan-500/30 px-3 py-1 rounded-full flex items-center gap-1.5">
              <CheckCircle2 size={13} className="text-secondary" /> Student Creator Hub
            </span>
          </div>
        ...
        <button
          ...
          className="bg-gradient-to-r from-violet-600 to-cyan-500 hover:from-violet-500 hover:to-cyan-400 text-white font-bold rounded-full px-7 py-3 flex items-center justify-center gap-2.5 shadow-[0_0_25px_rgba(168,85,247,0.45)] transition-all hover:scale-105 active:scale-95 cursor-pointer self-start sm:self-auto"
        >
          <Upload size={18} />
      ...
      {uploadSuccess && (
        <div className="p-4 rounded-2xl bg-emerald-500/15 border border-emerald-500/40 text-emerald-200 text-xs md:text-sm font-semibold flex items-center justify-between shadow-lg shadow-emerald-950/40">
          <span className="flex items-center gap-2">
            <CheckCircle2 size={18} className="text-emerald-400" />
            Track published successfully to CampusTunes!
          </span>
          <Link href="/campus" className="underline font-bold text-white hover:text-emerald-300 transition-colors">
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-on-surface-variant">Live streaming releases distributed to the campus</p>
          </div>
          <Link href="/campus" className="text-xs text-cyan-400 font-bold hover:underline flex items-center gap-1">
        ...
        {uploadedTracks.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {uploadedTracks.map((track, idx) => (
              <div
                key={track.id || idx}
                className="flex items-center gap-4 p-3.5 rounded-2xl bg-[#0f0e20] border border-white/[0.08] hover:border-violet-500/40 transition-all shadow-md group hover:-translate-y-0.5"
              >
                <div className="relative w-14 h-14 rounded-xl overflow-hidden shrink-0 border border-white/10 bg-[#161528]">
                  <Image
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <span className="text-[10px] text-violet-300 font-medium truncate">{track.genre || "Campus Track"}</span>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="rounded-3xl p-12 text-center border border-white/10 bg-[#0f0e20]/80 space-y-4 shadow-xl">
            <div className="w-16 h-16 rounded-2xl bg-violet-600/20 border border-violet-500/30 flex items-center justify-center text-primary mx-auto shadow-[0_0_20px_rgba(168,85,247,0.3)]">
              <FileAudio size={32} />
            </div>
            ...
            <button
              ...
              className="px-6 py-3 rounded-full bg-gradient-to-r from-violet-600 to-cyan-500 hover:opacity-90 text-xs font-bold text-white transition-all shadow-[0_0_20px_rgba(168,85,247,0.4)] cursor-pointer inline-flex items-center gap-2"
            >
      ...
      {/* Professional Track Distribution Modal (Mounted directly to document.body above all navbar & player docks) */}
      {isMounted &&
        showUploadModal &&
        createPortal(
              ...
              <div className="p-4 md:px-6 border-t border-white/10 bg-[#090814]/95 shrink-0 flex items-center justify-between gap-4 z-10">
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <button
                    ...
                    disabled={isUploading}
                    className="px-7 py-2.5 rounded-full text-xs font-bold bg-gradient-to-r from-violet-600 to-cyan-500 hover:from-violet-500 hover:to-cyan-400 text-white shadow-[0_0_20px_rgba(168,85,247,0.45)] transition-transform active:scale-95 disabled:opacity-50 cursor-pointer flex items-center gap-2"
                  >
                    {isUploading ? (
                      <>

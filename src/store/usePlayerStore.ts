...
export const REAL_CATALOG_SONGS: Song[] = [
  {
    id: "yt_tOM-nWPcR4U",
...
export const MOCK_SONGS: Song[] = REAL_CATALOG_SONGS;
...
export interface PlayerState {
  ...
  toggleLikeSong: (songOrId: Song | string) => void;
  syncLikesFromStorage: () => void;
  createPlaylist: (name: string, description?: string) => Playlist;
  deletePlaylist: (playlistId: string) => void;
  renamePlaylist: (playlistId: string, name: string) => void;
  addSongToPlaylist: (playlistId: string, song: Song) => void;
  removeSongFromPlaylist: (playlistId: string, songId: string) => void;
  syncPlaylistsFromStorage: () => void;
      ...
      try {
        ...
        if (raw) {
          ...
          if (Array.isArray(parsed)) {
            set({
              likedSongIds: new Set(parsed.map((s: Song) => s.id)),
              likedSongs: parsed,
            });
          }
        }
